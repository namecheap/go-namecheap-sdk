...
# testacc runs the build-tagged acceptance suite against the live Namecheap API.
# It needs NAMECHEAP_API_USER/NAMECHEAP_API_KEY/NAMECHEAP_CLIENT_IP (and
# optionally NAMECHEAP_USER_NAME and a disposable NAMECHEAP_TEST_DOMAIN) in the
# environment; without them the suite skips cleanly. NAMECHEAP_USE_SANDBOX=true
# points it at the sandbox endpoint instead of production. It is never part of
# `make test`.
testacc:
